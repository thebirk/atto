...
static void timer_handler(int unused)
{
    (void) unused;
    const int x = 1;
    write(timer_fds[1], &x, sizeof(int));
    alarm(1);
}
...
#define ENOUGH_DATA_FOR_PARSING 64
    ...
    while (1) {
        ...
        FD_SET(timer_fds[0], &events);
        //int maxfd = (winch_fds[0] > inout) ? winch_fds[0] : inout;
#define max(a, b) (a > b ? a : b)
        int maxfd = max(inout, max(winch_fds[0], timer_fds[0]));
        ...
        if (FD_ISSET(timer_fds[0], &events)) {
            event->type = TB_EVENT_TIMER;
            int x = 0;
            read(timer_fds[0], &x, sizeof(int));
